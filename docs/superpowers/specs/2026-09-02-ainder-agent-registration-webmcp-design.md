...
The backend remains authoritative for file count, size, detected MIME type, image decoding, ownership, and upload state.

## Shared Image Processing

Manual multipart uploads and Agent signed uploads feed the same server-side image processor. No original-format member photo is committed directly.

For each accepted source image, the processor:

1. accepts only decoded JPEG, PNG, or WebP input;
2. rejects files larger than 10 MiB;
3. applies EXIF orientation where relevant;
4. strips metadata;
5. scales proportionally until the image completely covers a 9:16 frame;
6. center-crops overflow without stretching or letterboxing;
7. outputs exactly 720 × 1280 pixels as WebP;
8. stores a server-generated random filename;
9. removes the temporary original after successful conversion or terminal failure.
...
The AI client sends the original image bytes directly to the signed URL. The signed endpoint verifies the signature and constraints, decodes and normalizes the image through the shared processor, then marks the upload `ready` only after the 720 × 1280 WebP has been committed to temporary registration storage.
